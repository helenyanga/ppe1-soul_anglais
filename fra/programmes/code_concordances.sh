...
for fichier in "$fichier_context"/fra-*.txt
do
    nom_fichier=$(basename "$fichier" .txt)
    N=$(echo "$nom_fichier" | sed 's/^fra-//')

    fichier_html_concordance="$fichier_concordance/fra-${N}.html"

    echo "Traitement des fichiers : $nom_fichier → fra-${N}.html"
    ...
    grep -o -i -E '.{0,50}\b(Â|â)me?\b.{0,50}' "$fichier" | while IFS= read -r line
