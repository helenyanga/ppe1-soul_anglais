        ...
        <table>
            <section>
                <div class="hero-body">
                    <div class="container">
                        <center>
                            <h1 class="title">Le mot « âme » en français (âme)<br><br></h1>
                        </center>
                   </div>
                </div>
            </section>
            <section class="section">
               <div class="container has-background-white">
                    <h1 class="title is-4">Tableau pour le corpus français - mot étudié ÂME :</h1>
                <table class="table">
                        <thead>
                            <tr>
                                <th>Numéro</th>
...
do
    ...
    fichier_data=$(curl -s -i -L -w "%{http_code}\n%{content_type}" -o "../aspirations/fra-${N}.html" $line) #Récupérer la page web avec ses métadonnées et les sauvegarder dans le fichier.
    ...
    nb_mots=$(lynx -dump -nolist "../aspirations/fra-$N.html" | wc -w)
    ...
    aspirations_fichier=$(curl -s -L "$line" -o "../aspirations/fra-${N}.html")
    dumps_fichier=$(lynx -dump -nolist "../aspirations/fra-${N}.html" > "../dumps-text/fra-${N}.txt")
    nb_occurences=$(egrep -a -i -o "\b(Â|a)mes?\b" "../dumps-text/fra-${N}.txt" | wc -l)
    contextes_fichier=$(egrep -a -i -C2 "\b(Â|a)mes?\b" "../dumps-text/fra-${N}.txt" > "../contextes/fra-${N}.txt")

    #Liens cliquables pour le tableau HTML :
    aspirations_fichier="../aspirations/fra-$N.html"
    dumps_fichier="../dumps-text/fra-$N.txt"
    contexte="../contextes/fra-${N}.html"
    concordances_fichier="../concordances/fra-${N}.html"
    ...
    echo -e "                   <tr>
                                    ...
                                    <td>$nb_mots</td>
                                    <td>$nb_occurences</td>
                                    <td><a href=\"../aspirations/fra-${N}.html\">aspiration</a></td>
                                    <td><a href=\"../dumps-text/fra-${N}.txt\">dump</a></td>
                                    <td><a href=\"../contextes/fra-${N}.txt\">contexte</a></td>
                                    <td><a href=\"../concordances/fra-${N}.html\">concordances</a></td>
                                </tr>" >> "$fichier_html"
...
cp "$fichier_tmp" "../tableaux/fra.tsv"
